...
NTSTATUS(NTAPI *NtProtectVirtualMemory)(HANDLE processHandle, PVOID *baseAddress, PSIZE_T regionSize, ULONG newAccessProtection);
NTSTATUS NTAPI NtProtectVirtualMemoryHook(HANDLE processHandle, PVOID *baseAddress, PSIZE_T regionSize, ULONG newAccessProtection) {
	if (processHandle == GetCurrentProcess() || !IsValidHandle(processHandle)) {
		return NtProtectVirtualMemory(processHandle, baseAddress, regionSize, newAccessProtection);
	}
...
VOID Attach() {
	...
	if (!ntdll) {
		MessageBox(0, L"[X] \"ntdll.dll\"", L"FAILURE", MB_ICONERROR);
		return;
	}
